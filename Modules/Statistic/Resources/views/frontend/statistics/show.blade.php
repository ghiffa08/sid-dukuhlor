                        ...
                        <div class="p-3">
                            <div class="space-y-1">
                                <a href="{{ route('frontend.statistik.index') }}" class="flex items-center rounded-xl px-4 py-3 text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-blue-600 transition-colors">
                                    <i class="fa-solid fa-list mr-3 text-gray-400 group-hover:text-blue-500"></i> Semua Statistik
                                </a>
                                 <a href="{{ route('frontend.statistik.show', Str::slug($statistic->category)) }}" class="flex items-center rounded-xl px-4 py-3 text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-blue-600 transition-colors">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Filter related stats to only show those in the same category
        // or just use the passed statistics. Assuming controller passes all stats or relevant ones.
        // If this is a single statistic view, maybe we want to compare with others in same category?
        // The controller passes $statistics = Statistic::active()->ordered()->get();
        // We should PROBABLY filter this in JS or better yet in Controller, but let's stick to View logic for now.
        // Let's filter client side to be safe if not filtered server side.
        var currentCategory = "{{ $statistic->category }}";
        var filteredStats = relatedStats.filter(item => item.category === currentCategory);

        var labels = filteredStats.map(item => item.title);
        var data = filteredStats.map(item => item.value);
        
        // Generate consistent colors
        var colors = filteredStats.map((item, index) => {
             const palette = ['#3b82f6', '#06b6d4', '#8b5cf6', '#10b981', '#f59e0b', '#ef4444'];
